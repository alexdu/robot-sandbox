...
def setRobotPos(J, grip_pos = -1):
    ...
    M["finger1"].setRot(P.getMat3().inverse().toList())
    M["finger2"].setRot(P.getMat3().inverse().toList())
...
def setGripperForces(open, close):
    if open:
        #M["finger1"].addForce((0,0,100), True)
        #M["finger2"].addForce((0,0,-100), True)
        slider_finger1.histop = 40
        slider_finger1.lostop = 40
        slider_finger2.histop = -40
        slider_finger2.lostop = -40
        
    if close:
        slider_finger1.histop = 10
        slider_finger1.lostop = 10
        slider_finger2.histop = -10
        slider_finger2.lostop = -10
#        M["finger1"].addForce((0,0,-100), True)
#        M["finger2"].addForce((0,0,100), True)
...
prop = ODEContactProperties(bounce = 0, mu = ode.Infinity, soft_erp=0.2, soft_cfm=0.001)
ode = ODEDynamics(gravity=9.81*100, substeps=1, defaultcontactproperties = prop)\
...
slider_finger1.motorfmax = 100
slider_finger2.motorfmax = 100
slider_finger1.cfm = 0.01
slider_finger2.cfm = 0.01
slider_finger1.stopcfm = 0.01
slider_finger2.stopcfm = 0.01
slider_finger1.stoperp = 0.8
slider_finger2.stoperp = 0.8
...
def randomizeBoxes():
    for b in boxes:
        ...
        if random.uniform(0,2) > 1:
            mb.setPos((random.uniform(-300,300), random.uniform(-400,-600), random.gauss(100,10)))
        else:
            mb.setPos((random.uniform(-300,300), random.uniform(400,600), random.gauss(100,10)))
