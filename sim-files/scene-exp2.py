...
def actGripper(open, close):
    if open:
        slider_finger1.motorvel = 100
        slider_finger2.motorvel = -100
    ...
    if close:
        ...
        #slider_finger1.lostop = pos-1
        #slider_finger2.histop = -pos+1
...
def tick():
    ...
    try:
        ...
        actGripper(RobotSim.sig_open, RobotSim.sig_close)
...
ode = ODEDynamics(gravity=9.81*100, substeps=10, cfm=1E-10, erp=0.1, defaultcontactproperties = prop)\
...
def randomizeBoxes():
    for b in boxes:
        ...
        if random.uniform(0,2) > 1:
            mb.setPos((random.uniform(-400,400), random.uniform(-300,-500), random.gauss(100,10)))
        else:
            mb.setPos((random.uniform(-400,400), random.uniform(300,500), random.gauss(100,10)))
